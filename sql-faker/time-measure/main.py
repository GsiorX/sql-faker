...
class Inmemory:
    ...
    def with_inmemory_column(self):
        ...
        class InMemory:
            def __enter__(self):
                cursor.execute("alter table " + table_name + " INMEMORY")
                cursor.execute("alter table " + table_name + " INMEMORY (" + col + ")")

                cursor.execute("exec DBMS_INMEMORY.POPULATE('C##ZSBD', '" + table_name + "');")

            def __exit__(self, exc_type, exc_val, exc_tb):
                cursor.execute("alter table " + table_name + " NO INMEMORY")
                cursor.execute("alter table " + table_name + " NO INMEMORY (" + col + ")")
    ...
    def with_inmemory_join_group(self):
        ...
        class InMemoryJoinGroup:
            def __enter__(self):
                cursor.execute("ALTER TABLE " + table_name + " INMEMORY")
                cursor.execute("ALTER TABLE " + table_name2 + " INMEMORY")
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                ...
                cursor.execute("ALTER TABLE " + table_name + " NO INMEMORY")
                cursor.execute("ALTER TABLE " + table_name2 + " NO INMEMORY")
        ...
        class InMemoryZa2:
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                try:
                    ...
                    print(1)
                cursor.execute("ALTER TABLE CREWMEMBER NO INMEMORY")
                cursor.execute("ALTER TABLE CREWMEMBER_JOB NO INMEMORY")
                cursor.execute("ALTER TABLE JOB NO INMEMORY")
                cursor.execute("ALTER TABLE CREWMEMBER_AWARD NO INMEMORY")
                cursor.execute("ALTER TABLE AWARD NO INMEMORY")
